...
//---------Restore parent data-----------------------------------------
    ...
    if(cur_pcb_ptr->pid_parent == -1){
        ...
        uint32_t eip_arg = cur_pcb_ptr->eip_user;
...
/* available_pid
 *   DESCRIPTION: find available pid
 *   INPUTS: None
 *   OUTPUTS: none
 *   RETURN VALUE: -1 for failure if no pid available; value between 0 and 7 for pid available;
 *   SIDE EFFECTS: none
 */
...
// PCB creation
    ...
    for (i = 0; i < 8;i++) {
        if(i==0){
            cur_pcb->file_array[i].fileop_ptr = &stdin_fileop_table;
            cur_pcb->file_array[i].used = 1;
        }
        else if(i==1){
            cur_pcb->file_array[i].fileop_ptr = &stdout_fileop_table;
            cur_pcb->file_array[i].used = 1;
        }
        else{
            cur_pcb->file_array[i].fileop_ptr = NULL;
