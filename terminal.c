...
void update_video_memory_paging(terminal_struct* next_term){
    ...
    // }
    pte[VIDEO_MEM_ADDR>>12].bit_addr_31_12 = (VIDEO_MEM_ADDR>>12) + 1 + next_term->terminal_num;
    flush_tlb(); // flush tlb
}
...
extern void switch_terminal(int terminal_num){
    ...
    // printf("t1");
    // if(terminal_array[terminal_num-1].number_of_processes==0 && available_pid()==-1){
    //     printf("No Available Process to Boot Terminal %d\n", terminal_num-1);
    //     return;
    // }

    int cur_pos = get_cursor_position();
    current_terminal->cursor_x = cur_pos%NUM_COLS;
...
int32_t terminal_read(int32_t fd, void* buf, int32_t nbytes){
    ...
    cli();
    for(i=0;i<current_terminal->command_idx;i++){
        command[i] = current_terminal->command[i];
    }
    command_idx = current_terminal->command_idx;
